...
namespace Sisus.Init.EditorOnly
{
	...
	internal static class AutoInitUtility
	{
		...
		internal static TArgument GetAutoInitArgument<TClient, TArgument>([DisallowNull] TClient client, int argumentIndex)
		{
			...
			return GetAutoInitArgument<TClient, TArgument>(client, from);
		}
		...
		/// <returns> Instance of type <typeparamref name="TArgument"/> or <see langword="null"/>. </returns>
		[return: MaybeNull]
		private static TArgument GetAutoInitArgument<TClient, TArgument>([DisallowNull] TClient client, From from)
		{
			...
			if(Service.TryGetFor(client, out TArgument service)
				// Skip services from other scenes, since Unity can't persist cross-scene references
				&& (Find.GameObjectOf(service) is not { } serviceGameObject
				|| Find.GameObjectOf(client) is not { } clientGameObject
				|| serviceGameObject.scene == clientGameObject.scene))
			{
				return service;
			}
		...
		private static bool ArgumentIsRequiredComponentFor([DisallowNull] Type argumentType, [DisallowNull] Type clientType)
		{
			...
			{
				#if DEV_MODE && DEBUG_ENABLED
				Debug.Log($"ArgumentIsRequiredComponentFor({argumentType.Name}, {clientType.Name}) : false - argument was not a component nor interface.");
				#endif
		...
		private static From GetFinalFromValue(From userProvidedValue, [DisallowNull] Type argumentType, bool isInitializer)
		{
			...
			if(Find.typesToFindableTypes.ContainsKey(argumentType))
			{
				...
				{
					return From.Children | From.Parent | From.SameScene;
				}
