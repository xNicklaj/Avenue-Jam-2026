...
namespace Sisus.Init
{
	...
	public sealed class InvalidInitArgumentsException : InitArgsException
	{
		...
		/// <summary>
		/// Checks if the <paramref name="argument"/> is <see langword="null"/> and assigns an <see cref="InvalidInitArgumentsException"/> object
		/// to <paramref name="exception"/> if it is.
		/// <para>
		/// This method call is ignored in non-development builds.
		/// </para>
		/// </summary>
		/// <param name="client"> Client receiving the argument. </param>
		/// <param name="argument"> The argument to test. </param>
		/// <param name="exception"> When this method returns, contains the exception if the argument was invalid; otherwise, <see langword="null"/>. </param>
		[Conditional("DEBUG"), Conditional("INIT_ARGS_SAFE_MODE"), MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static void ValidateNotNull<TArgument>([DisallowNull] object client, [MaybeNull] TArgument argument, [MaybeNull] ref InvalidInitArgumentsException exception)
		{
			if(argument as Object ?? argument is not null)
			{
				return;
