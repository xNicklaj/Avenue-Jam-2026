	...
	public static class Find
	{
		...
		public static void Components<T>([DisallowNull] List<T> results, bool includeInactive = false)
		{
			...
			for(int i = findableTypes.Length - 1; i >= 0; i--)
			{
				...
				TryAddAs(results, FindObjectsByType(findableType, includeInactive ? FindObjectsInactive.Include : FindObjectsInactive.Exclude));
		...
		public static GameObject GameObjectOf<T>([DisallowNull] T attachedObject)
		{
			...
			if(attachedObject is GameObject gameObject)
			{
				return gameObject;
			}
		...
		/// <summary>
		/// Gets the <see cref="UnityEngine.Object"/> associated with the <see paramref="@object"/>.
		/// </summary>
		/// <returns>
		/// Returns the <see paramref="@object"/> itself if it derives from <see cref="UnityEngine.Object"/>,
		/// the <see cref="Wrapper{T}"/> component of the given object, if it has one, or otherwise, <see langword="null"/>.
		/// </returns>
		[return: MaybeNull]
		public static Object UnityObjectOf([DisallowNull] object @object)
		{
			if(@object is Object unityObject)
			{
				return unityObject;
			}

			if(WrapperOf(@object, out var wrapper))
			{
				return wrapper.AsObject;
			}
		...
		/// </returns>
		[return: MaybeNull]
		public static bool UnityObjectOf([DisallowNull] object @object, out Object unityObject)
		{
			unityObject = @object as Object;
			if(unityObject)
			{
				return true;
			}
			...
			if(WrapperOf(@object, out var wrapper))
			{
				return unityObject = wrapper.AsObject;
			}
		...
		/// <summary>
		/// Gets the <see cref="Component"/> associated with the <see paramref="@object"/>.
		/// </summary>
		/// <returns>
		/// Returns the <see paramref="@object"/> itself if it derives from <see cref="Component"/>,
		/// the <see cref="Wrapper{T}"/> component of the given object, if it has one,
		/// the <see cref="Transform"/> attached to the given object, if it is a <see cref="GameObject"/>,
		/// or otherwise, <see langword="null"/>.
		/// </returns>
		[return: MaybeNull]
		public static Component ComponentOf([DisallowNull] object @object)
		{
			...
			if(WrapperOf(@object, out var wrapper))
			{
				return wrapper.AsMonoBehaviour;
			}
			
			if(@object is GameObject gameObject && gameObject)
			{
		...
		public static T Asset<T>()
		{
			...
			if(typeof(T).IsAbstract && typeof(Object).IsAssignableFrom(typeof(T)))
			{
				...
				foreach(var derivedType in TypeUtility.GetDerivedTypes(typeof(T)))
				{
		...
		private static Object[] ObjectsByExactType(Type findableType, bool includeInactive)
		{
			...
			#if UNITY_6000_4_OR_NEWER
			return FindObjectsByType(findableType, includeInactive ? FindObjectsInactive.Include : FindObjectsInactive.Exclude);
			#elif UNITY_2023_1_OR_NEWER
		...
		private static void Setup()
		{
			...
			var unityObjectTypes = TypeUtility.GetDerivedTypes(typeof(Object));
