...
{
	/// <summary>
	/// Returns all objects of the given type attached to the client's <see cref="GameObject"/> and all of its children.
	/// <para>
	...
	#endif
	{
		private const string MENU_NAME = "Hierarchy/Get Components In Children";
		...
		{
			foreach(var type in Find.typesToComponentTypes.Keys)
			{
				yield return type.MakeArrayType();
				yield return typeof(IReadOnlyList<>).MakeGenericType(type);
		...
		NullGuardResult INullGuardByType.EvaluateNullGuard<TValue>(Component client)
		{
			if(!client)
			{
				return NullGuardResult.Error($"{nameof(GetComponentsInChildren)} only supports clients attached to GameObjects.");
			}

			if(GetCollectionElementType(typeof(TValue)) is not { } elementType)
			{
				return NullGuardResult.Error($"{nameof(GetComponentsInChildren)} can only provide collection type values. {TypeUtility.ToString(typeof(TValue))} is not supported.");
			}

			if(!Find.typesToComponentTypes.ContainsKey(elementType))
			{
				return NullGuardResult.Error($"{nameof(GetComponentsInChildren)} can only provide values that are attachable to GameObjects. {TypeUtility.ToString(typeof(TValue))} is not supported.");
			}
