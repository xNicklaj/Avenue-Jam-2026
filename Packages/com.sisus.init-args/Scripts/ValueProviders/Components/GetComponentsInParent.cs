...
namespace Sisus.Init.ValueProviders
{
	...
	#endif
	{
		...
		NullGuardResult INullGuardByType.EvaluateNullGuard<TValue>(Component client)
		{
			if(!client)
			{
				return NullGuardResult.Error($"{nameof(GetComponentsInParent)} only supports clients attached to GameObjects.");
			}

			if(GetCollectionElementType(typeof(TValue)) is not { } elementType)
			{
				return NullGuardResult.Error($"{nameof(GetComponentsInParent)} can only provide collection type values. {TypeUtility.ToString(typeof(TValue))} is not supported.");
			}

			if(!Find.typesToComponentTypes.ContainsKey(elementType))
			{
				return NullGuardResult.Error($"{nameof(GetComponentsInParent)} can only provide values that are attachable to GameObjects. {TypeUtility.ToString(typeof(TValue))} is not supported.");
			}
