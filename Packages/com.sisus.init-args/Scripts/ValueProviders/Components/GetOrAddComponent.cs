...
{
	/// <summary>
	/// Returns an object of the given type attached to the client's <see cref="GameObject"/>,
	...
	#endif
	{
		...
		public bool TryGetFor<TValue>(Component client, out TValue value)
		{
			...
			return Find.In(client, out value) || TryAdd(client, out value);
		}
		...
		NullGuardResult INullGuardByType.EvaluateNullGuard<TValue>(Component client)
		{
			if(!client)
			{
				return NullGuardResult.Error($"{nameof(GetOrAddComponent)} only supports clients attached to GameObjects.");
			}
			...
			if(!Find.typesToComponentTypes.TryGetValue(typeof(TValue), out var componentTypes))
			{
				return NullGuardResult.Error($"{nameof(GetOrAddComponent)} only supports values that are attachable to GameObjects. {TypeUtility.ToString(typeof(TValue))} is not supported.");
			}

			if(componentTypes.Length is 1 || componentTypes.Any(x => x == typeof(TValue)))
			{
				return NullGuardResult.Passed;
			}
			...
			for(int i = componentTypes.Length - 1; i >= 0; i--)
			{
				...
				{
					return typeof(TValue).IsInterface
						? NullGuardResult.Error($"Ambiguous Match Exception: unable to determine type of the component to attach; both {typeToAdd.Name} and {componentType.Name} implement {typeof(TValue)}.")
						: NullGuardResult.Error($"Ambiguous Match Exception: unable to determine type of the component to attach; both {TypeUtility.ToString(typeToAdd)} and {TypeUtility.ToString(componentType)} derive from {typeof(TValue)}.");
				}
			...
			return typeToAdd is not null ? NullGuardResult.Passed : NullGuardResult.Error($"{nameof(GetOrAddComponent)} only supports values that are attachable to GameObjects. {TypeUtility.ToString(typeof(TValue))} is not supported.");
		}
		#endif
	}
}