...
namespace Sisus.Init.ValueProviders
{
	...
	#endif
	{
		...
		bool IValueByTypeProvider.HasValueFor<TValue>(Component client) => client && Find.typesToComponentTypes.ContainsKey(typeof(TValue));
		...
		NullGuardResult INullGuardByType.EvaluateNullGuard<TValue>(Component client)
		{
			if(!client)
			{
				return NullGuardResult.Error($"{nameof(AddComponent)} only supports clients attached to GameObjects.");
			}
			...
			{
				return NullGuardResult.Error($"{nameof(AddComponent)} only supports values that are attachable to GameObjects. {TypeUtility.ToString(typeof(TValue))} is not supported.");
			}
			...
			return typeToAdd is not null ? NullGuardResult.Passed : NullGuardResult.Error($"{nameof(AddComponent)} only supports values that are attachable to GameObjects. {TypeUtility.ToString(typeof(TValue))} is not supported.");
		}
		#endif
	}
}