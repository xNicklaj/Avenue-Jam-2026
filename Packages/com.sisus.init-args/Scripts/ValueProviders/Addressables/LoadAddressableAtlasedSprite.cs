		...
		#endif
	{
		...
		GetForAsync([AllowNull] Component client, CancellationToken cancellationToken = default)
		{
			...
			if(clients.Add(client) && clients.Count == 1)
			{
				...
				var loadTask = atlasedSprite.LoadAssetAsync().Task;
				await loadTask;
		...
		NullGuardResult INullGuard.EvaluateNullGuard([AllowNull] Component client)
		{
			if(!atlasedSprite.RuntimeKeyIsValid())
			{
				return NullGuardResult.Error("Addressable key is invalid.");
			}
			
			if(string.IsNullOrEmpty(atlasedSprite.SubObjectName))
			{
				return NullGuardResult.Error($"Addressable '{atlasedSprite.RuntimeKey}' sub-object name is missing.");
			}

			if(atlasedSprite.OperationHandle.IsValid() && atlasedSprite.OperationHandle.OperationException is { } exception)
			{
				return NullGuardResult.Exception("Loading addressable failed: " + exception);
			}

			var spriteAtlas = atlasedSprite.editorAsset;
			if(!spriteAtlas)
			{
				return NullGuardResult.Error($"Sprite atlas of addressable '{atlasedSprite.RuntimeKey}' not found.");
			}

			var sprite = spriteAtlas.GetSprite(atlasedSprite.SubObjectName);
			if(!sprite)
			{
				return NullGuardResult.Error($"Sprite '{atlasedSprite.SubObjectName}' not found in sprite atlas '{spriteAtlas.name}'.");
			}
