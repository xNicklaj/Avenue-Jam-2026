...
namespace Sisus.Init
{
	...
	public interface IValueReleaser<in TValue> : IValueReleaser
	{
		/// <summary>
		...
		/// <inheritdoc/>
		void IValueReleaser.Release(Component client, object value) => Release(client, (TValue)value);
	}

	/// <summary>
	/// Non-generic base interface for <see cref="IValueReleaser{TValue}"/>.
	/// </summary>
	public interface IValueReleaser
	{
		/// <summary>
		/// Returns the value that was being used by the <paramref name="client"/>.
		/// </summary>
		/// <param name="client">
		/// The object that requested the value for the value provider, or <see langword="null"/> if the requester
		/// was unknown or was not a <see cref="Component"/>.
		...
		/// </param>
		void Release([AllowNull] Component client, object value);
	}
}