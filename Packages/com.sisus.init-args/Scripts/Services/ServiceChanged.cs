...
namespace Sisus.Init
{
	...
	internal delegate void ServiceProviderChangedHandler<TService>(Clients clients, [AllowNull] ServiceProvider<TService> old, [AllowNull] ServiceProvider<TService> newInstance);
	...
	internal static class ServiceChanged<TService>
	{
		...
		internal static readonly Dictionary<Action, ServiceChangedHandler<TService>> handlers = new();
	}

	/// <summary>
	/// Class that holds the current subscribers to the <see cref="ServiceProviderChangedHandler{TService}"/> event.
	/// </summary>
	/// <typeparam name="TService">
	/// The defining type of the service class, which is the type specified in its <see cref="ServiceAttribute"/>,
	/// or - if no other type has been explicitly specified - the exact type of the service class.
	/// </typeparam>
	internal static class ServiceProviderChanged<TService>
	{
		/// <summary>
		/// Event that is invoked when a globally available shared instance of service of type
		/// <typeparamref name="TService"/> has changed to another one.
		/// </summary>
		internal static ServiceProviderChangedHandler<TService> listeners;

		internal static readonly Dictionary<Action, ServiceProviderChangedHandler<TService>> handlers = new();
	}
}