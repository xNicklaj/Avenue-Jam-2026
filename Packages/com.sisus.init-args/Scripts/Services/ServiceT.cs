	...
	public static class Service<TService>
	{
		...
		static Service()
		{
			...
			if(Service.nowSettingInstance != typeof(TService)
			   && ServiceInjector.TryGetUninitializedServiceInfo(typeof(TService), out var serviceInfo)
			   && !serviceInfo.IsTransient
			   && serviceInfo.HasClosedConcreteType)
			{
				#if UNITY_EDITOR
				...
				_ = 
				#endif
				ServiceInjector.LazyInit(serviceInfo, typeof(TService), client: null)
				#if DEBUG
					.OnFailure(ServiceInjector.HandleLogException);
				#endif
					;
			}
			#endif
