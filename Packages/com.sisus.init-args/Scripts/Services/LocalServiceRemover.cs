...
{
	internal sealed class LocalServiceRemover
	{
		...
		static LocalServiceRemover()
		{
			const BindingFlags flags = BindingFlags.Static | BindingFlags.Public;
			var methods = typeof(Service).GetMember(nameof(Service.RemoveFrom), flags);

			#if DEV_MODE
			// RemoveFrom<TService>(Clients clients, TService service, Component serviceProvider),
			// RemoveFrom<TService>(Clients clients, Component serviceProvider)
			// and 7 value provider specific ones.
			Debug.Assert(methods.Length == 9, $"Unexpected RemoveFrom method count: {methods.Length}.");
			#endif
			...
			foreach(MethodInfo method in methods)
			{
				...
				if(parameterType.IsGenericType)
				{
					...
					#if DEV_MODE
					Debug.LogError($"RemoveFrom method {method} with unrecognized generic type definition {genericTypeDefinition} detected.");
					#endif
				}
			...
			Debug.Assert(forServiceMethodDefinition != null, "MethodInfo Service.RemoveFrom<TService>(object instance, TService service, Clients forClients) not found.");
			Debug.Assert(forValueProviderTMethodDefinition != null, "MethodInfo Service.RemoveFrom<TService>(object instance, IValueProvider<TService> serviceProvider, Clients forClients) not found.");
			Debug.Assert(forValueProviderTAsyncMethodDefinition != null, "MethodInfo Service.RemoveFrom<TService>(object instance, IValueProviderAsync<TService> serviceProvider, Clients forClients) not found.");
			Debug.Assert(forValueByTypeProviderMethodDefinition != null, "MethodInfo Service.RemoveFrom<TService>(object instance, IValueByTypeProvider serviceProvider, Clients forClients) not found.");
			Debug.Assert(forValueByTypeProviderAsyncMethodDefinition != null, "MethodInfo Service.RemoveFrom<TService>(object instance, IValueByTypeProviderAsync serviceProvider, Clients forClients) not found.");
			Debug.Assert(forValueProviderMethodDefinition != null, "MethodInfo Service.RemoveFrom<TService>(object instance, IValueProvider serviceProvider, Clients forClients) not found.");
			Debug.Assert(forValueProviderAsyncMethodDefinition != null, "MethodInfo Service.RemoveFrom<TService>(object instance, IValueProviderAsync serviceProvider, Clients forClients) not found.");
			#endif
		}

		public LocalServiceRemover([DisallowNull] Type definingType, ServiceProviderType serviceProviderType)
		{
			Type delegateType;
		...
		public void RemoveFrom([DisallowNull] object serviceOrProvider, Clients forClients, [DisallowNull] Component registerer)
		{
