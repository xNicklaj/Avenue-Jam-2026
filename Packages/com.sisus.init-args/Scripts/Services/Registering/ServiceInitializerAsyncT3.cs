...
namespace Sisus.Init
{
	...
	public abstract class ServiceInitializerAsync<TService, TFirstArgument, TSecondArgument, TThirdArgument> : IServiceInitializerAsync<TService, TFirstArgument, TSecondArgument, TThirdArgument>
	{
		/// <inheritdoc/>
		async Task<object> IServiceInitializerAsync.InitTargetAsync(params object[] arguments)
		{
			...
			else if(arguments.Length != 4) Debug.LogWarning($"{GetType().Name}.{nameof(InitTargetAsync)} was given {arguments.Length} services, when four were expected.");
			#endif

			return await InitTargetAsync((TFirstArgument)arguments[0], (TSecondArgument)arguments[1], (TThirdArgument)arguments[2], (CancellationToken)arguments[3]);
		}

		/// <inheritdoc/>
		public abstract Task<TService> InitTargetAsync(TFirstArgument firstArgument, TSecondArgument secondArgument, TThirdArgument thirdArgument, CancellationToken cancellationToken);
	}
}