...
namespace Sisus.Init
{
	...
	public abstract class ServiceInitializerAsync<TService, TFirstArgument, TSecondArgument> : IServiceInitializerAsync<TService, TFirstArgument, TSecondArgument>
	{
		/// <inheritdoc/>
		async Task<object> IServiceInitializerAsync.InitTargetAsync(params object[] arguments)
		{
			#if DEBUG || INIT_ARGS_SAFE_MODE
			if(arguments is null) Debug.LogWarning($"{GetType().Name}.{nameof(InitTargetAsync)} was given no services, when two were expected.");
			...
			return await InitTargetAsync((TFirstArgument)arguments[0], (TSecondArgument)arguments[1], (CancellationToken)arguments[2]);
		}

		/// <inheritdoc/>
		public abstract Task<TService> InitTargetAsync(TFirstArgument firstArgument, TSecondArgument secondArgument, CancellationToken cancellationToken);
	}
}