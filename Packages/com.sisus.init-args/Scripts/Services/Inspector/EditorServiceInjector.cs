...
namespace Sisus.Init.Internal
{
	...
	{
		internal static bool ServiceInitializationInProgress { get; private set; }
		...
		private static void CreateAndInjectServices()
		{
			...
			ServiceInitializationInProgress = true;

			var services = CreateInstancesOfAllServices();

			ServiceInitializationInProgress = false;

			InjectServiceDependenciesForTypesThatRequireOnlyThem(services);
		...
		private static void CreateService(Dictionary<Type, object> services, Type classWithAttribute, EditorServiceAttribute serviceAttribute, ref GameObject containerGameObject)
		{
			...
			SetInstance(definingType, instance);
		...
		/// </exception>
		private static void SetInstance(Type definingType, object instance)
		{
		...
		private static void InjectServiceDependenciesForTypesThatRequireOnlyThem([AllowNull] Dictionary<Type, object> services)
		{
			...
			foreach(var clientType in TypeCache.GetTypesDerivedFrom<IOneArgument>())
			{
			...
			foreach(var clientType in TypeCache.GetTypesDerivedFrom<ITwoArguments>())
			{
				if(clientType.IsAbstract)
				{
					continue;
			...
			foreach(var clientType in TypeCache.GetTypesDerivedFrom<IThreeArguments>())
			{
				if(clientType.IsAbstract)
				{
					continue;
			...
			foreach(var clientType in TypeCache.GetTypesDerivedFrom<IFourArguments>())
			{
				if(clientType.IsAbstract)
				{
					continue;
			...
			foreach(var clientType in TypeCache.GetTypesDerivedFrom<IFiveArguments>())
			{
				if(clientType.IsAbstract)
				{
					continue;
		...
		private static Dictionary<int, MethodInfo> GetInitArgsSetMethods()
		{
			...
			Dictionary<int, MethodInfo> setMethodsByArgumentCount = new(MAX_INIT_ARGUMENT_COUNT);
