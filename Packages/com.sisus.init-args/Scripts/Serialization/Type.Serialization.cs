...
namespace Sisus.Init.Serialization
{
	...
	/// </summary>
	[Serializable]
	internal sealed class _Type : ISerializationCallbackReceiver, IValueProvider<Type>
	{
		private static readonly HashSet<string> typeNotFoundWarningHandledFor = new HashSet<string>();

		[SerializeField]
		internal string scriptAssetGuid;

		[SerializeField]
		internal string TypeNameAndAssembly;
		...
		{
			Value = !string.IsNullOrEmpty(assemblyQualifiedTypeName) ? Type.GetType(assemblyQualifiedTypeName, throwOnError: false) : null;
		...
		public override string ToString() => Value?.FullName is null ? TypeNameAndAssembly : Value.Name;
		...
		void ISerializationCallbackReceiver.OnAfterDeserialize()
		{
			...
			{
				value = Type.GetType(TypeNameAndAssembly, throwOnError: false);
