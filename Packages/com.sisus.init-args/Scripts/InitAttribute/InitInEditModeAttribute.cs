...
namespace Sisus.Init
{
	...
	/// classes that derive from <see cref="MonoBehaviour{T}"/>. When this is done, dependencies for the component are automatically
	/// retrieved from the scene hierarchy, relative to the <see cref="GameObject"/> that contains the component with the attribute.
	/// You can specify <see cref="From"/> where <see cref="UnityEngine.Object"/> arguments should be searched, or use the
	...
	/// <item> other <see cref="UnityEngine.Object"/>: <see cref="From.Assets"/>, </item>
	/// <item>
	/// Collection of <see cref="Component"/>, interface, <see cref="Transform"/> or <see cref="GameObject"/>: <see cref="From.Children"/>.
	/// For example <see cref="Collider"/>[] or <see cref="System.Collections.Generic.IEnumerable{Transform}"/>.
	/// </item>
	/// </list>
	/// </para>
	/// <para>
	/// The found arguments are passed to the component's <see cref="IInitializable{TArgument}.Init"/> function,
	...
	/// needing to assign all <see cref="UnityEngine.Object"/> references manually using the inspector.
	/// </para>
	/// <para>
	/// This attribute can also be added to classes that derive from one of the <see cref="Initializer{TClient, TArgument}"/> base classes.
	...
	{
		/// <summary>
		/// When this attribute is attached to a <see cref="MonoBehaviour{TArgument}"/> class, instances of the
		...
		/// using the initializer are passed to the <see cref="IInitializable{TArgument}.Init"/> function of the client.
		/// </para>
		/// <para>
		...
		/// and passed to its <see cref="IInitializable{TArgument}.Init"/> function.
		/// <para>
		...
		/// <see cref="UnityEngine.Object"/> references manually through the inspector.
		/// </para>
		/// </summary>
		...
		/// <summary>
		/// When a component that derives from <see cref="MonoBehaviour{TArgument}"/> and has the
		/// <see cref="InitInEditModeAttribute"/> is first added to a GameObject in the editor,
		/// or when the user hits the Reset button in the Inspector's context menu,
		/// the arguments accepted by the component are automatically gathered
		/// and passed to its <see cref="IInitializable{TArgument}.Init"/> function.
