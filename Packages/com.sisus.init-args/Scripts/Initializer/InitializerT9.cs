...
{
	/// <summary>
	/// A base class for a component that can specify the nine arguments used to initialize a component that implements
	...
	public abstract class Initializer<TClient, TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument, TSixthArgument, TSeventhArgument, TEighthArgument, TNinthArgument>
		: InitializerBase<TClient, TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument, TSixthArgument, TSeventhArgument, TEighthArgument, TNinthArgument>
			where TClient : MonoBehaviour, IInitializable<TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument, TSixthArgument, TSeventhArgument, TEighthArgument, TNinthArgument>
	{
		...
		InitTargetAsync(TClient target)
		{
			...
			var seventhAwaitable = this.seventhArgument.GetValueAsync(this, cancellationToken: cancellationToken);
			var eighthAwaitable = this.eighthArgument.GetValueAsync(this, cancellationToken: cancellationToken);
			var ninthAwaitable = this.ninthArgument.GetValueAsync(this, cancellationToken: cancellationToken);
			...
			var seventhArgument = await seventhAwaitable;
			var eighthArgument = await eighthAwaitable;
			var ninthArgument = await ninthAwaitable;
			...
			if(target.gameObject != gameObject)
			{
				...
				var results = await target.InstantiateAsync(firstArgument, secondArgument, thirdArgument, fourthArgument, fifthArgument, sixthArgument, seventhArgument, eighthArgument, ninthArgument);
				return results[0];
				#else
		...
		private protected sealed override void Reset()
		{
				...
				&& sharedInstance is WaitForService waitForService)
			{
				...
				HandleMakeWaitForService(this, ref fourthArgument, waitForService);
				HandleMakeWaitForService(this, ref fifthArgument, waitForService);
				HandleMakeWaitForService(this, ref sixthArgument, waitForService);
				HandleMakeWaitForService(this, ref seventhArgument, waitForService);
				HandleMakeWaitForService(this, ref eighthArgument, waitForService);
				HandleMakeWaitForService(this, ref ninthArgument, waitForService);
			}
		}
		#endif
	}
