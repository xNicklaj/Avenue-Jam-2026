...
namespace Sisus.Init.Internal
{
	...
	public static class InitializableUtility
	{
		...
		private static readonly Dictionary<Type, bool> hidesAwakeCache = new(64);
		...
		/// Does the client derive from base class that can handle automatically initializing itself with all services?
		/// </summary>
		public static bool CanSelfInitializeWithoutInitializer([DisallowNull] Component client) => client is IInitializable;
		...
		public static bool HidesAwake(Object target) => HidesAwake(target.GetType());

		public static bool HidesAwake(Type targetType)
		{
			if (hidesAwakeCache.TryGetValue(targetType, out var hidesAwake))
			{
				return hidesAwake;
			}

			for (var type = targetType; !TypeUtility.IsNullOrBaseType(type); type = type.BaseType)
			{
				if (hidesAwakeCache.TryGetValue(type, out hidesAwake))
				{
					break;
				}

				if (type.GetMember("Awake", BindingFlags.DeclaredOnly | BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
					.Any(m => m is MethodInfo method && method.GetParameters().Length is 0))
				{
					hidesAwake = TypeUtility.DerivesFromGenericBaseType(type);
					break;
				}
			}

			hidesAwakeCache.Add(targetType, hidesAwake);
			return hidesAwake;
		}
	}
}