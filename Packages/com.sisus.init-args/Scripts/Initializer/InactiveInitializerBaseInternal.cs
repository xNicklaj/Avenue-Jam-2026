...
namespace Sisus.Init.Internal
{
		...
		where TClient : Component
	{
		...
		IEnumerable<Type> IValueByTypeProvider.GetSupportedValueTypes()
		{
			...
			{
				yield return typeof(TClient);
				yield break;
			}

			yield return typeof(TClient);

			foreach(var baseType in typeof(TClient).GetBaseTypes())
			{
				yield return baseType;
			}

			foreach(var interfaceType in typeof(TClient).GetInterfaces())
			{
		...
		private protected virtual void EvaluateNullGuard(List<NullGuardResult> failures) { }
