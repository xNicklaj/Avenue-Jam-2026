	...
	public sealed class InactiveInitializer : InactiveInitializerBaseInternal<MonoBehaviour>, IInitializable
	{
		...
		private static void OnEnterPlayMode()
		{
			...
			foreach(var inactiveInitializer in
				...
				FindObjectsByType<InactiveInitializer>(FindObjectsInactive.Include))
		...
		private protected override void EvaluateNullGuard(List<NullGuardResult> failures)
		{
			if(!target)
			{
				failures.Add(NullGuardResult.ErrorBox($"{nameof(InactiveInitializer)} target is missing."));
			}
			else if(target is not IInitializable)
			{
				failures.Add(NullGuardResult.ErrorBox($"{nameof(InactiveInitializer)} target {target.GetType().Name} does not implement {nameof(IInitializable)}."));
			}
		}
