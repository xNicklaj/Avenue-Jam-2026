...
namespace Sisus.Init.Internal
{
		...
		where TClient : ScriptableObject
	{
		...
		private protected static NullGuardResult EvaluateNullGuard<T>(T argument) => IsNull(argument) ? NullGuardResult.Error($"Argument of type {TypeUtility.ToString(typeof(T))} is missing.") : NullGuardResult.Passed;
		...
		{
			if(argument.EvaluateNullGuard() is { Type: not NullGuardResultType.Passed } failure) failures.Add(failure);
		}
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
