...
namespace Sisus.Init.Internal
{
	...
	public static class InitializerUtility
	{
		...
		public static bool TryGetClientAndInitArgumentTypes(Type initializerType, [MaybeNullWhen(false), NotNullWhen(true)] out Type clientType, [MaybeNullWhen(false), NotNullWhen(true)] out Type[] initArgumentTypes)
		{
			if(initializerType.GetInterfaces().Select(GetFromInterface).OrderByDescending(x => x.initArgumentTypes.Length).FirstOrDefault() is { clientType: not null} result)
			{
				clientType = result.clientType;
				initArgumentTypes = result.initArgumentTypes;
				return true;
			}
			...
			(Type clientType, Type[] initArgumentTypes) GetFromInterface(Type interfaceType)
			{
				if(interfaceType.IsGenericType && argumentCountsByIInitializerTypeDefinition.ContainsKey(interfaceType.GetGenericTypeDefinition()))
				{
					...
					return (genericArguments[0], genericArguments.Skip(1).ToArray());
				}
		...
		private static bool IsAsyncValueProviderOrService<TArgument>(Any<TArgument> argument)
		{
			...
			if(ServiceAttributeUtility.TryGetInfoForDefiningType(typeof(TArgument), out var serviceInfo) &&
				(serviceInfo.LoadAsync
				// Initialize clients of scene based services asynchronously even if LoadAsync is set to false, because Unity's scene loading system is inherently at least a little bit async (single frame delay). 
				|| serviceInfo.referenceType is ReferenceType.SceneName or ReferenceType.SceneBuildIndex))
			{
				return true;
			}
