...
namespace Sisus.Init
{
		...
		#endif
	{
		...
		/// </para>
		/// </returns>
		public bool TryGetValue([MaybeNullWhen(false), NotNullWhen(true)] out T value) => TryGetValue(null, Context.MainThread, out value);
		...
		public bool TryGetValue([AllowNull] Component client, Context context, [MaybeNullWhen(false), NotNullWhen(true)] out T result)
		{
			// Prefer cached value over a value provider - this can help avoid issues
			// such as addressable assets being loaded more than once by the same client
			// and causing the internal counter to get incremented too many times.
		...
		/// </param>
		/// <returns> <see langword="true"/> if a valid underlying value has been assigned; otherwise, <see langword="false"/>. </returns>
		public bool TryGetValue([AllowNull] Component client, [MaybeNullWhen(false), NotNullWhen(true)] out T value) => TryGetValue(client, Context.MainThread, out value);

		/// <summary>
		/// Attempts to get the underlying value provider of the specified type.
		/// </summary>
		/// <param name="valueProvider"> When this method returns, contains the underlying value provider if found; otherwise, <see langword="null"/>. </param>
		/// <typeparam name="TValueProvider"> The type of value provider to try and get. </typeparam>
		/// <returns> <see langword="true"/> if the underlying value provider was found; otherwise, <see langword="false"/>. </returns>
		public bool TryGetValueProvider<TValueProvider>([MaybeNullWhen(false), NotNullWhen(true)] out TValueProvider valueProvider) where TValueProvider : class
		{
			valueProvider = reference as TValueProvider;
			return valueProvider is not null;
		}
