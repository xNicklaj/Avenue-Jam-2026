...
namespace Sisus.Init
{
	/// <summary>
	/// <para>
	/// Represents a non-reference value of type <typeparamref name="T"/> that can be assigned
	/// through Unity's inspector as well as serialized by its serializer.
	/// </para>
	/// <para>
	/// <see cref="AnyValue{T}"/> can be used instead of <see cref="Any{T}"/> when <see typeparamref="T"/> is a value type,
	/// to avoid the error 'The field Sisus.Init.Any{SomeStruct}.value has the [SerializeReference] attribute applied, but is of type SomeStruct, which is a value type. By definition, value types cannot be serialized by reference.'
	/// </para>
	/// </summary>
	/// <remarks>
	/// <para>
	...
	/// <see cref="AnyValue{T}"/> can wrap an instance of this class and return its <see cref="IValueProvider{T}.Value"/>
	/// when <see cref="Value"/> is called.
	/// </para>
	/// <para>
	/// If a class derives from <see cref="Object"/> and implements <see cref="IValueByTypeProvider"/> then
	/// <see cref="AnyValue{T}"/> can wrap an instance of this class and use <see cref="IValueByTypeProvider.TryGetFor"/>
		...
		#endif
	{
		...
		/// <summary>
		/// Gets a value indicating whether the current <see cref="AnyValue{T}"/> object
		/// has a non-null underlying type.
		/// <para>
		/// <see langword="true"/> if the current <see cref="AnyValue{T}"/> object has a value;
		/// <see langword="false"/> if the current <see cref="AnyValue{T}"/> object has no value.
		/// </para>
		/// </summary>
		public bool HasValue => GetHasValue(null);

		/// <summary>
		/// Gets the value of the current <see cref="AnyValue{T}"/> object
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="AnyValue{T}"/> struct with the given underlying value.
		/// </summary>
		/// <param name="value"> The underlying value of the <see cref="AnyValue{T}"/> object. </param>
		...
		/// </summary>
		/// <param name="value">
		/// The underlying value of the <see cref="AnyValue{T}"/> object.
		/// </param>
		...
		/// <summary>
		/// Creates a new instance of the <see cref="AnyValue{T}"/> struct with the given underlying value.
		/// </summary>
		/// <param name="value"> The underlying value of the <see cref="AnyValue{T}"/> object. </param>
		/// <returns> A new instance of the <see cref="AnyValue{T}"/> struct. </returns>
		public static AnyValue<T> FromValue(T value) => new(value);
		...
		/// </param>
		/// <returns> A new instance of the <see cref="AnyValue{T}"/> struct. </returns>
		public static AnyValue<T> FromObject([AllowNull] Object value) => new(value);

		/// <summary>
		/// Returns a value indicating whether it is possible to create an instance of the
		/// <see cref="AnyValue{T}"/> struct with the underlying value of <paramref name="fromReference"/>.
		/// </summary>
		/// <param name="fromReference"> The underlying value of the <see cref="AnyValue{T}"/> object. </param>
		/// <returns> A new instance of the <see cref="AnyValue{T}"/> struct. </returns>
		...
		/// <summary>
		/// Defines an implicit conversion of <see cref="AnyValue{T}"/> to the <see cref="T"/> object that it wraps.
		/// </summary>
		/// <param name="instance"> The <see cref="AnyValue{T}"/> instance to convert. </param>
		public static implicit operator T(AnyValue<T> instance) => instance.Value;

		/// <summary>
		/// Defines an explicit conversion of <see cref="T"/> to an <see cref="AnyValue{T}"/> object.
		/// </summary>
		/// <param name="value"> The value to convert. </param>
		public static implicit operator AnyValue<T>(T value) => new(value);
		...
		/// <returns>
		/// The value associated with the current <see cref="AnyValue{T}"/> object, if it has a value;
		/// otherwise, the default value of <see cref="T"/>.
		/// </returns>
		...
		/// <summary>
		/// Gets the value of the current <see cref="AnyValue{T}"/> object if it has one.
		/// <para>
		...
		/// <summary>
		/// Gets a value indicating whether the current <see cref="AnyValue{T}"/> object has a non-null value
		...
		/// </param>
		/// <returns>
		/// <see langword="true"/> if the <see cref="AnyValue{T}"/> object has a non-null value.
		/// <para>
		...
		/// <summary>
		/// Gets the current value of the <see cref="AnyValue{T}"/> object if it has one.
		/// <para>
		/// This method can only be called from the main thread.
		/// </para>
		/// </summary>
		/// <param name="value">
		/// When this method returns, contains the current value of the <see cref="AnyValue{T}"/> object, if it has been assigned one.
		/// <para>
		...
		/// </summary>
		/// <param name="value">
		/// When this method returns, contains the value associated with the current <see cref="AnyValue{T}"/> object, if it has a value;
		...
		/// <returns> <see langword="true"/> if the specified object is equal to the current object; otherwise, <see langword="false"/>. </returns>
		public bool Equals(AnyValue<T> other) => reference == other.reference && EqualityComparer<T>.Default.Equals(value, other.value);
		...
		/// <summary>
		/// Returns the hash code for this <see cref="AnyValue{T}"/> object.
		/// </summary>
		...
		private static void AssertConstructorArgumentIsValid(Object reference)
		{
			...
			if(!reference)
			{
				...
				{
					throw new InvalidCastException($"{nameof(AnyValue<bool>)}<{typeof(T).Name}> can not have a null value because {typeof(T).Name} is a value type.");
				}
			...
			throw new InvalidCastException($"{nameof(AnyValue<bool>)}<{typeof(T).Name}> can not have a value of type {reference.GetType().Name}.");
			#endif
		}
		...
		private static void AssertSerializedReferenceIsValid(ref Object reference)
		{
			...
			Debug.LogWarning($"{nameof(AnyValue<bool>)}<{TypeUtility.ToStringNicified(typeof(T))}> can not have a value of type {TypeUtility.ToStringNicified(reference.GetType())}.", reference);
			reference = null;
			#endif
		}
