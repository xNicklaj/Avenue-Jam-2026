...
namespace Sisus.Init
{
	...
	public abstract class Wrapper<TWrapped> : Wrapper, IWrapper<TWrapped>, IValueProvider<TWrapped>, IValueByTypeProvider
	{
		...
		{
			if(typeof(TWrapped) == typeof(object))
			{
				if(wrapped is null)
				{
					foreach(var type in TypeUtility.GetAllTypesThreadSafe())
					{
						yield return type;
					}
				...
				var wrappedObjectType = wrapped.GetType();
				yield return wrappedObjectType;

				foreach(var baseType in wrappedObjectType.GetBaseTypes())
				{
					yield return baseType;
				}

				foreach(var interfaceType in wrappedObjectType.GetInterfaces())
				{
