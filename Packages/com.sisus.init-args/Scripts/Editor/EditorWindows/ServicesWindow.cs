		...
		private void DrawServicesList(List<Service.ActiveServiceInfo> activeServices, IEnumerable<Service.ActiveServiceInfo> inactiveServices)
		{
			...
			void DrawService(Service.ActiveServiceInfo serviceInfo)
			{
				...
				if(!serviceInfo.IsSetupDone)
				{
					...
					{
						serviceInfo.Setup(services.providesServices.Where(IsDefinitionForService).Select(x => x.DefiningType).Where(x => x is not null).ToArray());
						bool IsDefinitionForService(ServiceDefinition definition)
						{
							if(ReferenceEquals(definition.service, service))
							{
								return true;
							}

							if(definition.service is IWrapper wrapper)
							{
								return ReferenceEquals(wrapper.WrappedObject, service);
							}
							
							if(definition.service is IInitializer initializer)
							{
								return ReferenceEquals(initializer.Target, service);
							}
