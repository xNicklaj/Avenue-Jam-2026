	...
	internal static class ComponentMenuItems
	{
		...
		private static bool IsGenerateInitializerMenuItemClickable(MenuCommand command)
		{
			return command.context && (command.context is Component
				...
				&& !InitializerEditorUtility.HasAnyInitializerTypes(command.context.GetType(), matchInitializersForDerivedAndInterfaceTypes: false);
		}
		...
		private static bool IsToggleShowShowInitSectionClickable(MenuCommand command)
		{
			...
			if(InitAttributeUtility.TryGet(targetType, out var initAttribute) && initAttribute.HideInInspector)
			{
				return false;
			}

			return InitializerEditorUtility.IsInitializable(targetType) || InitializerEditorUtility.HasAnyInitializerTypes(targetType, matchInitializersForDerivedAndInterfaceTypes: true);
		}
	}
}