	...
	public abstract class MultiInitializableEditorDecorator<TTarget, TClient> : EditorDecorator where TTarget : Component where TClient : class
	{
		...
		private void RebuildInitializerGUIs(TClient[] firstInitializables, out bool drawInitializerGUI)
		{
			...
			foreach(TClient initializable in firstInitializables)
			{
				...
				foreach(var initializerType in InitializerEditorUtility.GetInitializerTypes(concreteType, matchInitializersForDerivedAndInterfaceTypes: true))
				{
		...
		private void OnAddButtonPressed(Rect addRect)
		{
			...
			{
				Type concreteType = initializable.GetType();
				var initializerTypes = InitializerEditorUtility.GetInitializerTypes(concreteType, matchInitializersForDerivedAndInterfaceTypes: true).ToArray();
		...
		private bool HasAnyAddableInitializerTypes()
		{
			...
			foreach(TClient initializable in tempClientsList)
			{
				foreach(var initializerType in InitializerEditorUtility.GetInitializerTypes(initializable.GetType(), matchInitializersForDerivedAndInterfaceTypes: true))
				{
		...
		private bool HasAnyInitializablesWithoutAnInitializerClass()
		{
			...
			foreach(TClient initializable in tempClientsList)
			{
				if(!InitializerEditorUtility.HasAnyInitializerTypes(initializable.GetType(), matchInitializersForDerivedAndInterfaceTypes: true))
				{
					tempClientsList.Clear();
					return true;
				}
