...
namespace Sisus.Init.EditorOnly
{
	...
	public class InitializableEditorDecorator : EditorDecorator
	{
		...
		private static void OnScriptsReloaded()
		{
			...
			var entityIds = value.Substring(i + 1).Split(';');
			foreach(var entityIdString in entityIds)
			{
				if(!int.TryParse(entityIdString, out int entityIdAsInt))
				{
					#if DEV_MODE
					Debug.LogWarning($"!int.TryParse({entityIdString})");
					#endif
					continue;
				}

				#if UNITY_6000_4_OR_NEWER
				var target = EditorUtility.EntityIdToObject(EntityId.FromULong((ulong)entityIdAsInt));
				#else
		...
		protected virtual bool ShouldDrawAsRuntimeField([DisallowNull] FieldInfo field) => RuntimeFieldsDrawer.ShouldDrawAsRuntimeField(field);
		...
		[Pure]
		protected virtual RuntimeFieldsDrawer CreateRuntimeFieldsDrawer() => new(target, BaseType, ShouldDrawAsRuntimeField);
