...
{
	/// <summary>
	/// Custom property drawer for <see cref="Any{T}"/> and <see cref="AnyValue{T}"/>,
	...
	internal sealed class AnyPropertyDrawer : PropertyDrawer
	{
		...
		private float DrawValueField(Rect position, SerializedProperty anyProperty, State state, GUIContent label)
		{
			...
			if(drawAsObjectField)
			{
				...
				if(referenceValue)
				{
					if(referenceValue is CrossSceneReference crossSceneReference)
					{
						...
						else
						{
							...
							return valueProperty is not null ? EditorGUI.GetPropertyHeight(valueProperty, label, true) : EditorGUIUtility.singleLineHeight;
						}
					}
				...
				if(state.draggedObjectIsAssignable is true && CrossSceneReference.Detect(firstTargetObject, DragAndDrop.objectReferences[0]))
				{
					...
					CrossSceneReferenceGUI.OverrideObjectPicker(controlRect, state.referenceProperty, state.valueType, state);
					var visualizedReferenceValue = state.CurrentObjectPicker ? state.ObjectPickerCurrentValue : referenceValue;
					Object setReferenceValue = EditorGUI.ObjectField(controlRect, GUIContent.none, visualizedReferenceValue, state.objectFieldConstraint, true);

					GUI.color = guiColorWas;

					if(!state.CurrentObjectPicker && setReferenceValue != referenceValue && setReferenceValue && TryGetAssignableType(state, setReferenceValue, out _))
					{
		...
		#if DEV_MODE && DEBUG_CROSS_SCENE_REFERENCES
		private static string GetScene(Object obj) => obj switch
		{
			GameObject gameObject => gameObject.scene.name,
			Component component => component.gameObject.scene.name,
			_ => "n/a"
		};
		#endif

		private sealed class State : IObjectPickerPreviewable
		{
			...
			private void SetUserSelectedValueType(Type setType)
			{
				...
				else if(typeof(Object).IsAssignableFrom(setType))
				{
					...
					else if(setType.GetCustomAttribute<ValueProviderMenuAttribute>() is { } valueProviderMenuAttribute
							&& typeof(ScriptableObject).IsAssignableFrom(setType))
					{
						...
						if(useSingleSharedInstance)
						{
							referenceProperty.objectReferenceValue = singleSharedInstance;
						}
						else
						{
							#if DEV_MODE
							Debug.Log($"Creating new embedded instance of value provider {setType.Name}, because no shared single instance was found in the project.");
							#endif

							var embeddedInstance = ScriptableObject.CreateInstance(setType);
							var valueProviderName = valueProviderMenuAttribute.ItemName;
							if(string.IsNullOrEmpty(valueProviderName))
							{
								valueProviderName = TypeUtility.ToStringNicified(setType);
							}

							ValueProviderEditorUtility.Assign(referenceProperty, embeddedInstance, valueProviderName);
						}
					}
				}
