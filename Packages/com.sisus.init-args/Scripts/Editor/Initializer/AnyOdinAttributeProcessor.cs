//#if AUTO_INJECT_REQUIRED_ATTRIBUTE
...
namespace Sisus.Init.EditorOnly.Internal
{
	internal sealed class AnyOdinAttributeProcessor<T> : OdinAttributeProcessor<Any<T>>
	{
		...
		public override void ProcessChildMemberAttributes(InspectorProperty parentProperty, MemberInfo member, List<Attribute> attributes)
		{
			...
			{
				#if INJECT_REQUIRED_ATTRIBUTE
				if(!attributes.Any(x => x is RequiredAttribute)
				&& parentProperty.Parent.ValueEntry.WeakSmartValue is IInitializerEditorOnly initializerEditorOnly
				...
				if(InitializerEditorUtility.GetMetaDataClassType(maybeInitializerType) is { } metadataClass
