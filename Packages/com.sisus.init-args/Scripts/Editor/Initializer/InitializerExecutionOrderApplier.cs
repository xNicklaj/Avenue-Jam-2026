...
namespace Sisus.Init
{
	...
	internal sealed class InitializerExecutionOrderApplier
	{
		...
		public void UpdateExecutionOrderOfAllInitializers()
		{
			...
			for(; i >= 1; i--)
			{
				...
				if(isCloseEnoughToPreviousOrder
					&& differenceBetweenPreviousAndNext is <= 1 or > STEP_SIZE + STEP_SIZE)
				{
					#if DEV_MODE && DEBUG_SET_ORDER
				...
				if(differenceBetweenPreviousAndNext is > 1 and <= STEP_SIZE + STEP_SIZE)
				{
					#if DEV_MODE && DEBUG_SET_ORDER
		...
		{
			if(InitializerUtility.TryGetClientAndInitArgumentTypes(initializerType, out Type clientType, out Type[] argumentTypes))
			{
				clientTypes[initializerType] = clientType;
				genericArgumentTypes[initializerType] = argumentTypes;
				return (clientType, argumentTypes);
			}
		...
		private Type GetClientType(Type initializerType) => clientTypes.TryGetValue(initializerType, out Type result) ? result : GetAndCacheClientAndParameterTypes(initializerType).clientType;
