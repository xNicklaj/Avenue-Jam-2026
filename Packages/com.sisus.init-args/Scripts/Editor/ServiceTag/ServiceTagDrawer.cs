...
namespace Sisus.Init.EditorOnly.Internal
{
	...
	internal static class ServiceTagDrawer
	{
		...
		private static readonly Dictionary<Clients, GUIContent> mouseoverLabels = new()
		{
			...
			{ Clients.InAllScenes, new("All Scenes") },
		...
		private static void OnBeforeHeaderGUI(Component[] targets, Rect headerRect, bool headerIsSelected, bool supportsRichText)
		{
			...
			if(EditorServiceTagUtility.openSelectTagsMenuFor == serviceOrServiceProvider)
			{
				...
				EditorServiceTagUtility.OpenSelectDefiningTypesMenu(serviceOrServiceProvider, tagRect);
				return;
			}
		...
		private static GUIContent GetTagLabel(Component serviceOrServiceProvider, Rect headerRect, Span<Type> definingTypes)
		{
			...
			var toClients = EditorServiceTagUtility.GetClients(serviceOrServiceProvider);
		...
		{
			foreach(var referenceable in component.gameObject.GetComponentsNonAlloc<IdTag>())
			{
