...
namespace Sisus.Init.EditorOnly
{
    internal sealed class AdvancedDropdownGUI
    {
        ...
        public void DrawItem(DropdownItem item, bool selected, bool hasSearch)
        {
            if(item.IsSeparator())
            {
                DrawLineSeparator();
        ...
        {
            var rect = GUILayoutUtility.GetRect(EditorGUIUtility.currentViewWidth, 5f);
            rect.y += 2f;
            rect.height = 1f;
            rect.x += 10f;
            rect.width -= 20f;
        ...
        public void DrawHeader(DropdownItem group, Action backButtonPressed)
        {
